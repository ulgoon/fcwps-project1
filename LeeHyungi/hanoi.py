def hanoi(n, disk_stack, through_stack, to_stack):
    if n > 0:
        # disk_stack에 있는 원반을 through_stack으로 옮겨준다.
        # 큰 원반을 제외한 원반 이동
        hanoi(n-1, disk_stack, through_stack, to_stack)
        ...
        hanoi(n-1, through_stack, disk_stack, to_stack)
...
hanoi(len(disk_stack), disk_stack, through_stack, to_stack)
print(f"disk_stack : {disk_stack}")
